...
1. The iris dataset contains 150 observations of 5 variables.
```{r}
library(tidyverse)
data("iris")
head(iris)
...
2. The iris1 dataset now contains 56 observations of 5 variables.
```{r}
iris1<-filter(iris, Species %in% c("virginica", "versicolor"),Sepal.Length > 6.0 ,Sepal.Width > 2.5) 

```

3.The iris1 dataset now contains 56 observations of 3 variables.
```{r}
...
5. The iris4 dataset now contains 56 observations of 4 variables.
```{r}
